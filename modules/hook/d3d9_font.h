// filepath: modules/hook/d3d9_font.h
// Compact 8x8 bitmap font and 2D vertex primitives for Direct3D 9 HUD overlay.
#pragma once
...
constexpr DWORD kFontFVF = D3DFVF_XYZRHW | D3DFVF_DIFFUSE;

// Simple, compact 8x8 font table for ASCII 32 (' ') to 126 ('~')
static const uint8_t kFont8x8[95][8] = {
    ...
    {0x0E,0x18,0x7C,0x18,0x18,0x18,0x18,0x00}, // 102 'f'
...
inline void AddQuad(std::vector<Vertex2D>& verts, float x, float y, float w, float h, D3DCOLOR c) {
    verts.push_back({ x,     y,     0.0f, 1.0f, c });
    verts.push_back({ x + w, y,     0.0f, 1.0f, c });
    verts.push_back({ x + w, y + h, 0.0f, 1.0f, c });
    verts.push_back({ x,     y,     0.0f, 1.0f, c });
    verts.push_back({ x + w, y + h, 0.0f, 1.0f, c });
    verts.push_back({ x,     y + h, 0.0f, 1.0f, c });
}

inline void AddChar(std::vector<Vertex2D>& verts, char ch, float x, float y, float scale, D3DCOLOR c) {
    if (ch < 32 || ch > 126) ch = ' ';
    ...
    for (int row = 0; row < 8; ++row) {
        ...
        for (int col = 0; col < 8; ++col) {
            if (bits & (0x80 >> col)) {
                AddQuad(verts, x + col * scale, y + row * scale, scale, scale, c);
            }
        }
    }
}

inline void AddString(std::vector<Vertex2D>& verts, const std::string& str, float x, float y, float scale, D3DCOLOR c) {
    float cur_x = x;
    for (char ch : str) {
        if (ch == '\n') {
            y += 9.0f * scale;
        ...
        AddChar(verts, ch, cur_x, y, scale, c);
        cur_x += 8.0f * scale;
    }
}
